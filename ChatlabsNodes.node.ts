  ...
  description: INodeTypeDescription = {
    ...
    properties: [
      ...
      },
      {
        ...
        options: [
          ...
          },
          {
            ...
            description:
              "Busca automaticamente um atendente online no departamento",
          },
        ],
      ...
      },
      {
        ...
        placeholder: "1,2,3",
        description:
          "Filtra atendentes por cargo. Informe IDs separados por vírgula (opcional)",
      },
      {
        ...
        default: "",
        description:
          "Destino de fallback caso nenhum atendente esteja disponível (opcional)",
      },
      {
      ...
      try {
        ...
        if (resource === "chat") {
          if (operation === "list") {
            ...
            for (const [param, key] of strFields) {
              const val = this.getNodeParameter(param, i) as string;
              if (val) qs[key] = val;
            }
          ...
          if (operation === "transferChat") {
            ...
            const transferType = this.getNodeParameter(
              "transferType",
              i,
            ) as string;
            const hasTransferMessage = this.getNodeParameter(
              "hasTransferMessage",
              i,
            ) as boolean;
            ...
            if (transferType === "TRANSFER_TO_ATTENDANT") {
              body.attendantId = this.getNodeParameter(
                "transferAttendantId",
                i,
              ) as string;
            } else if (transferType === "TRANSFER_TO_DEPARTMENT") {
              body.departmentId = this.getNodeParameter(
                "transferDepartmentId",
                i,
              ) as string;
            } else if (transferType === "TRANSFER_TO_AVAILABLE_ATTENDANT") {
              const deptId = this.getNodeParameter(
                "transferAvailableDeptId",
                i,
              ) as string;
              const roleIdsRaw = this.getNodeParameter(
                "transferRoleIds",
                i,
              ) as string;
              const fallbackType = this.getNodeParameter(
                "transferFallbackType",
                i,
              ) as string;

              const transferToAvailableAttendant: IDataObject = {
                departmentId: deptId,
              };
              ...
              if (fallbackType) {
                const fallbackId = this.getNodeParameter(
                  "transferFallbackId",
                  i,
                ) as string;
                transferToAvailableAttendant.fallback = {
                  type: fallbackType,
                  id: fallbackId,
                };
              }
        ...
        if (resource === "tag") {
          ...
          if (operation === "create") {
            ...
            const color = this.getNodeParameter("tagColor", i) as string;
            const fullname = this.getNodeParameter("tagFullname", i) as string;
            const code = this.getNodeParameter("tagCode", i) as string;
