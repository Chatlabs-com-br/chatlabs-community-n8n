            ...
            try {
                ...
                if (resource === "chat") {
                    ...
                    if (operation === "transferChat") {
                        const chatId = this.getNodeParameter("transferChatId", i);
                        const transferType = this.getNodeParameter("transferType", i);
                        const hasTransferMessage = this.getNodeParameter("hasTransferMessage", i);
                        const body = { type: transferType, hasTransferMessage };
                        if (transferType === "TRANSFER_TO_ATTENDANT") {
                            body.attendantId = this.getNodeParameter("transferAttendantId", i);
                        }
                        else if (transferType === "TRANSFER_TO_DEPARTMENT") {
                            body.departmentId = this.getNodeParameter("transferDepartmentId", i);
                        }
                        else if (transferType === "TRANSFER_TO_AVAILABLE_ATTENDANT") {
                            const deptId = this.getNodeParameter("transferAvailableDeptId", i);
                            const roleIdsRaw = this.getNodeParameter("transferRoleIds", i);
                            const fallbackType = this.getNodeParameter("transferFallbackType", i);
                            const transferToAvailableAttendant = {
                                departmentId: deptId,
                            };
                            if (roleIdsRaw) {
                            ...
                            if (fallbackType) {
                                const fallbackId = this.getNodeParameter("transferFallbackId", i);
